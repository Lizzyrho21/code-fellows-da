...
const addOne = (arr) => {
  let newarr = []; // new array to push numbers inside
  // Solution code here...
  arr.forEach((el) => {
    // foreach element...
    let incremented = el + 1; // increment value in variable
    newarr.push(incremented); // push inside array
    // return array once done
  });
  return newarr;
};
...
const addExclamation = (arr) => {
  let newStringArr = []; //empty array
  // Solution code here...
  arr.forEach((element) => {
    let excited = element + '!';
    newStringArr.push(excited);
  });
  return newStringArr;
};
...
const allUpperCase = (arr) => {
  ...
  let uppercaseIt = [];
  arr.forEach((el) => {
    let changed = el.toUpperCase();
    uppercaseIt.push(changed);
  });
  return uppercaseIt;
};
...
const greeting = (word) => {
  // Solution code here...
  let upperIt = word.toUpperCase() + '!';
  return upperIt;
};

const speaker = (words, callback) => {
  let newArrOfStrings = [];// we set up a place for our strings to be returned.
  // Solution code here...
  words.forEach((word) => { // for each word in the string array
    newArrOfStrings.push(callback(word)); // send it to our greeting and push it into our new array
  });
  return newArrOfStrings; // return at the end!
};
